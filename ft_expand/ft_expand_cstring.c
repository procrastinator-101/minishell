...
/*   Updated: 2021/06/07 10:43:33 by yarroubi         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "ft_expand.h"

static char	*ft_get_cstring(char *str, char *quotes, int *len)
{
	int		i;

	i = 2;
	while (str[i] && (str[i] != '\'' || quotes[i] != UNQUOTED))
		i++;
	*len = i;
...
char	*ft_expand_cstring(char *str, char *quotes, int *len)
{
	...
	if (!ret)
		return (0);
	cstring = ft_get_cstring(str, quotes, len);
	if (!cstring)
		free(ret);
	if (!cstring)
		return (0);
