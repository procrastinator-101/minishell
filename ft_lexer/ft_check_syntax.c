...
/*   Updated: 2021/05/30 11:19:21 by yarroubi         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	ft_check_syntax(t_token *tokens)
{
	...
	if (!tokens)
		return (0);
	head = tokens;
	if (ft_isoperator(head) && !ft_isredirection(head))
		return (ECCO + (head->type << 8));
	while (head)
	{
		next = head->next;
		if (!next && (ft_isredirection(head) || head->type == PIPE))
			return (ECCO + (NEW_LINE << 8));
		else if (next && ft_isoperator(head) && ft_isoperator(next))
		{
			if (ft_isredirection(head))
