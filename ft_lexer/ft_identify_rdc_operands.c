...
/*   Updated: 2021/05/30 11:53:29 by yarroubi         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
			...
			t_token *token, t_token *next)
{
	...
	{
		if (previous->type == WORD && next && ft_isredirection(next))
			previous->type = RDC_LOPERAND;
		previous->next = next;
	}
}
...
void	ft_identify_rdc_operands(t_token **tokens)
{
	...
	while (token)
	{
		...
		{
			if (token->type == WORD && previous && ft_isredirection(previous))
				token->type = RDC_ROPERAND;
			else if (ft_isredirection(token) && next && next->type == WORD)
				next->type = RDC_ROPERAND;
			previous = token;
		}
		token = next;
	}
